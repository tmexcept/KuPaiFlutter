...
class CustomPaintPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => PaintIndexChange(),
      child: Scaffold(
        appBar: AppBar(
          title: Text("自定义绘制"),
          elevation: 0.0,
        ),
        body: CustomPaintShow(),
              ...
              ? InkWell(
                  ...
                  onTap: () async {
                    ...
                    var images = await getImage(
                        'assets/Activity_.png'); // 使用方法获取Unit8List格式的图片
                    // 绘制图片
                    ...
                    ParagraphConstraints pc =
                        ui.ParagraphConstraints(width: images.width.toDouble());
                    ...
                    var picture = await pictureRecorder
                        .endRecording()
                        .toImage(750, 1334); //设置生成图片的宽和高
                    var pngImageBytes = await picture.toByteData(
...
Map<String, CustomPainter> paintDemos = {
  'PaintDemo1': PaintDemo1(),
  'PaintDemo2': PaintDemo2(),
  'PaintDemo3': PaintDemo3(),
  'PaintDemo4': PaintDemo4(),
  'PaintDemo5': PaintDemo5(),
  'PaintDemo6': PaintDemo6(),
  'PaintDemo7': PaintDemo7(),
  'RadarDemo': RadarDemo(6),
};

class PaintIndexChange with ChangeNotifier {
  int _index = 0;
...
class CustomPaintShow extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    PaintIndexChange change = Provider.of<PaintIndexChange>(context);
    ...
    return Container(
      ...
      children: <Widget>[
        CustomPaint(
          ...
          foregroundPainter: paintLists.elementAt(change.index),
        )
      ],
    ));
  }
...
class PaintDemo5 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    rect2 = Rect.fromCircle(center: Offset(200.0, 250.0), radius: 80.0);
...
class PaintDemo9 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path = Path()..moveTo(300.0, 100.0);
    rect = Rect.fromCircle(center: Offset(300.0, 100.0), radius: 60.0);
...
class PaintDemo15 extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    getImage("image/Activity_.png").then((value) {
...
class RadarDemo extends CustomPainter {
  ...
  late double c_X; // view 的中心点
  late double c_Y;
  late double maxRadius; // 半径，最大的半径
  late Paint linePaint; // 划线的画笔
  late Path path; // 路径
  late Paint maskPaint; // 遮罩的画笔

  late double eachRadius;
  late double eachAngle;
  ...
  @override
  void paint(Canvas canvas, Size size) {
    print("RadarDemo.paint width=${size.width}, height=${size.height}");
    c_X = size.width / 2;
    c_Y = size.height / 2;
    if (c_X > c_Y) {
      maxRadius = c_Y;
...
class SpiderNetView extends CustomPainter {
  ...
  ///view 的中心点
  late double viewCenterX;
  late double viewCenterY;
...
class MyFloatingActionButton extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    var change = Provider.of<PaintIndexChange>(context);
    print("创建 MyFloatingActionButton build index=${change.index}");
    return FloatingActionButton(
      onPressed: () {
        if (change.index < paintLists.length - 1) {
          change.index += 1;
        } else {
          change.index = 0;
        }
      ...
      tooltip: 'Change  Type',
      child: Text("index=${change.index}", textAlign: TextAlign.center,),
    );
  }
}
